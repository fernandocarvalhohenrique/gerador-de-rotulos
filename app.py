...
# --- BANCO DE DADOS SQLITE (COM MIGRAÇÃO AUTOMÁTICA E PERMISSÕES) ---
def init_db():
    conn = sqlite3.connect("rotulos_app.db")
    c = conn.cursor()
    
    # 1. Cria a tabela de usuários caso não exista
    c.execute('''
    ...
    # 2. Garante a criação/existência da coluna 'is_admin'
    try:
...
if st.session_state.is_admin:
    ...
    with tab_admin:
        ...
        if opcoes_remocao:
            ...
            st.info("Nenhum outro usuário disponível para remoção.")
